$(function () {
        ...
        identify: function () {
            ...
            if ( Remote.party && Remote.party !== '' ) {
                ...
                Remote.socket.emit( 'identify', {
                    type: 'remote',
                    partyPlace: Remote.party
                } );

                Remote.partyPlace.removeClass('hidden')
                        .html( 'controlling <a href="/remote/party/'+ Remote.party +'">'+ Remote.party +'</a>' );
    ...
    if ( window.io && window.gifPartyConfig && window.gifPartyConfig.party ) {
        Remote.init( socketServer, window.gifPartyConfig.party );
    } else {
        $('.loader' ).html('unable to connect to GifParty Remote');
    }
});
